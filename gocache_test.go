...
func TestGetter(t *testing.T) {
	// define a func f as a Getter, then call f.Get() is actually calling the func itself
...
func TestGet(t *testing.T) {
	...
	for k, v := range db {
		if view, err := g.Get(k); err != nil || view.String() != v {
			t.Fatalf("failed to get value of %s, should be %s, but %s got", k, v, view.String())
		}
